...
网站默认读取 `learning/` 中的 50 篇学习稿。全部章节均按问题、心智模型、连续机制推演、失效边界、工程验证和自测完成深度重写，网站会根据 Markdown 状态实时显示 `50 / 50`。`content/` 只作为本地来源层保留，不参与公开站点发布。
...
学习稿审计会核对 50 篇课程一一对应、每篇均有完整推演与自测、不引用来源图片，并拒绝来源水印、个人品牌、宣传信息、私有区图标和不可见控制字符进入学习层。
